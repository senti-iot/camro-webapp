		...
		if (sessionCookie) {
			...
			if (authStatus === 200) {
				...
				if (userData) {
					...
					if (userData.internal && userData.internal.camro && userData.internal.camro.favorites) {
						dispatch({
							type: setFavoritesData,
							payload: userData.internal.camro.favorites
						});
					}
				}
			} else {
...
const saveFavorites = (noConfirm) => {
	return async (dispatch, getState) => {
		...
		internal.camro = internal.camro || {};
		internal.camro.favorites = f || [];
